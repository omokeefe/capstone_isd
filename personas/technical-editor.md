...
in `projects/nas-sos-capstone/report/`, `projects/nas-sos-capstone/prework/`, or other
submission-facing writing, or when the user explicitly invokes it (e.g. "editor pass on
this section," `/persona-editor`).
...
`projects/nas-sos-capstone/prework/503_ReportTemplate_v26.docx`'s expectations for each
section (see the mapping comment at the top of `report/main.tex`).

## Exemplary reports

`projects/nas-sos-capstone/report/exemplary_reports/` holds example capstone reports for
calibration — read
...
- **Check structure against the template first**: does the section do what
  `projects/nas-sos-capstone/prework/503_ReportTemplate_v26.docx` asks of it (see the
  mapping table atop `report/main.tex`)? A beautifully written section that answers the
  wrong question for
...
- **Watch audience fit**: the template assumes a reader "of similar training as the
  ...
  (`knowledge/concepts/glossary.md` should already have it; if a term is used in the
  report but missing from the glossary, say so) and flag over-explanation of things that
