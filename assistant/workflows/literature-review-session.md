...
For a session where the goal is to make a dent in `Project_To-Do List.md` §2, §3, or §4
...
1. Bootstrap per `assistant/workflows/new-session-bootstrap.md`.
2. Open `assistant/memory/source-register.md`, filter to the relevant to-do section
   ...
   flag in `open-questions.md` if a source can't be located quickly.
3. For each source, run `assistant/workflows/annotate-source.md`.
4. After the batch, do a **synthesis pass** across the sources just read:
   ...
   - Did any source suggest a stakeholder objective, conflict, or decomposition angle
     not yet in `stakeholder-register.md` or `project-brief.md`? Add it.
5. Check off the relevant boxes in `Project_To-Do List.md` for each source processed.
6. Wrap up per `assistant/workflows/session-wrap-up.md`, noting in the session log which
