...
Skim (don't narrate this part): `assistant/memory/project-brief.md`,
`assistant/tasks/task-board.md`, and today's date. This is a fast read, not the full
`new-session-bootstrap.md` pass — skip `open-questions.md` and `decisions-log.md` unless
something in the status check below needs them.

## Step 2 — EPM status check (`persona-pm`)

Follow `assistant/personas/project-manager.md`. Walk `Project_To-Do List.md`'s ECD-tagged
sections against today's date and classify each as **done / on track / at risk /
slipped** (definitions in that persona file — "done" requires the stated verification
metric to plausibly be met, not just that the date passed).
...
`Project_To-Do List.md` has no dated sections yet (e.g. dates haven't been assigned), say
...
Follow `assistant/personas/professor.md`, but keep it short — a few sentences, not a full
...
`assistant/journal/YYYY-MM-DD.md` per `assistant/journal/README.md`'s format — today's
date, a `## HH:MM — Tag-Up` heading (real wall-clock time, e.g. via `date +%H:%M`, not a
guess). If today's journal file doesn't exist yet, create it with the `# Journal —
YYYY-MM-DD` header first. If it exists, read it and append rather than overwriting
earlier entries.
...
- If a real schedule change or replan comes out of this (the user decides to re-scope, or
  accepts a slip and moves a date), update `assistant/tasks/task-board.md` accordingly
  per the PM persona's own rule.
