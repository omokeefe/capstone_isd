...
A fast triage pass over `evidence/sources/` — distinct from (and a prerequisite to) the
deep extraction in `workflows/annotate-source.md`. Run this whenever the user says
something like "process my references" / "check for new references." The goal: every
file in `evidence/sources/` has a complete bib entry, a short written summary, and an
honest 0-5 rating, and the register knows when it was last checked.

## Step 1 — Find what's new

1. Read `evidence/source-register.md`'s **Processing Ledger** — note the "Last
   full sweep" date and which files already have an entry.
2. List the actual contents of `evidence/sources/` (top-level files only; skip
   `evidence/literature-notes/summaries/` and `evidence/literature-notes/annotations/` —
   those are outputs, not sources). Compare against the ledger.
...
2. **Bibliography** — check `evidence/sources/references.bib` for an existing stub entry
...
4. **Write the summary** — copy
   `templates/reference-summary-template.md` to
   `evidence/literature-notes/summaries/<bib-key-or-slug>.md` and fill it in:
   ...
   - What's valuable in it and for which part of the project — be specific: does it feed
     a `to-do-list.md` section (§2-§4 literature review, §6 decomposition, §7-§9
...
5. **Update the ledger row** in `evidence/source-register.md`: bib key, rating,
...
7. If the summary surfaced something belonging in `knowledge/concepts/glossary.md`,
   `knowledge/models/stakeholder-register.md`, or `knowledge/questions/open-questions.md`,
...
- If `evidence/literature-notes/summaries/` doesn't exist yet, create it and add a
  one-line `evidence/literature-notes/summaries/README.md` noting these are generated by
  this workflow.
