...
`evidence/source-register.md` plus a standalone annotation note, satisfying the
"Read and annotate paper" checklist items throughout `to-do-list.md` §2–§4.
...
1. Open `evidence/source-register.md` and find the source's row (add one if
   missing — file + bib key + type + status).
2. Copy `templates/source-annotation-template.md` and fill it in while
...
3. Save the filled-in note. Suggested location: alongside the source, e.g.
   `evidence/literature-notes/annotations/<short-name>.md` (create the `annotations/`
...
5. If the source surfaced a new term, add it to `knowledge/concepts/glossary.md`.
6. If the source surfaced a new stakeholder, decision, or objective not already in
   `knowledge/models/stakeholder-register.md`, add it there (or flag it as a follow-up
   in `knowledge/questions/open-questions.md` if it needs a judgment call first).
7. Go back to `projects/nas-sos-capstone/to-do-list.md` and check off the completed
   sub-bullets for that source. Leave "Map findings to candidate SysML elements" (or
   equivalent) unchecked until that mapping is actually done — annotation and
   architecture-mapping are separate steps.
8. If this was the last unannotated source in a literature-review batch, consider running
   `workflows/literature-review-session.md`'s wrap-up step to synthesize across
